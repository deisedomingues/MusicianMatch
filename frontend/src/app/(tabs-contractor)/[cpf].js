...
export default function MusicianProfile() {
  ...
  const loadMusicianData = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/musicos/${cpf}`);
  ...
  const loadAvaliacoes = async () => {
    try {
      ...
      const response = await api.get(`/avaliacoes/musico/${cpf}`);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.infoBox}>
        ...
        <Info label="Descrição" value={musician.bio || "Sem descrição."} />
      ...
      {loadingAvaliacoes ? (
        ...
        <Text style={{ color: "#aaa", marginBottom: 20 }}>Nenhuma avaliação ainda.</Text>
      ) : (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 25 }}>
          {avaliacoes.map((av, index) => (
            <View key={index} style={styles.avaliacaoCard}>
              ...
              <Text style={styles.avaliacaoComentario}>{av.comentario || "Sem comentário"}</Text>
              <Text style={styles.avaliacaoAutor}>Por: {av.cpf_contratante}</Text>
            </View>
          ))}
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  loadingContainer: { flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#160427" },
  ...
  avatar: { width: 110, height: 110, borderRadius: 55, backgroundColor: "#3A0E5A", justifyContent: "center", alignItems: "center", marginBottom: 12 },
  name: { color: "#fff", fontSize: 24, fontWeight: "bold" },
  instrument: { color: "#BC9CFF", fontSize: 16, marginTop: 4 },
  location: { color: "#aaa", fontSize: 14, marginTop: 2 },
  infoBox: { backgroundColor: "#2b0e47", padding: 20, borderRadius: 14, marginTop: 10, marginBottom: 25 },
  ...
  avaliacaoCard: { backgroundColor: "#2b0e47", padding: 16, borderRadius: 12, marginRight: 12, width: 250 },
  ...
  mainButton: { flexDirection: "row", backgroundColor: "#7B1FA2", paddingVertical: 16, borderRadius: 12, justifyContent: "center", alignItems: "center", marginBottom: 40 },
  mainButtonText: { color: "#fff", fontSize: 18, fontWeight: "bold", marginLeft: 10 },
});
