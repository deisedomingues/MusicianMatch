...
export class ContratacaoController {
  ...
  // dentro de ContratacaoController (substitua a função create atual)
async create(req, res) {
  try {
    ...
    // garantir nomes: se não vierem, buscar na tabela Usuario
    if (!nome_musico) {
    ...
    // tratamento de data (mesma lógica sua, mantida)
    if (data_evento) {
    ...
    const query = `
      INSERT INTO Contratacao
        (cpf_contratante, cpf_musico, nome_contratante, nome_musico, instrumentos, data_evento, horario, localizacao, observacoes, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    // retornar o registro criado (id) e os dados mínimos
